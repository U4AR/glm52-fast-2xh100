...
def kt_adaptive_on_decode_step() -> None:
    """Compatibility hook for model_runner's optional adaptive-cache tick.

    GSI v1 deliberately uses frozen expert residency and frozen activation
    bases.  The model runner in the pinned environment calls this hook
    unconditionally, while this isolated KT wrapper does not include RunGLM's
    experimental online cache implementation.  Keep the static path a cheap
    no-op and reject an accidental request for online adaptation explicitly.
    """
    if os.environ.get("KT_ADAPTIVE_DECODE") == "1":
        raise RuntimeError(
            "KT_ADAPTIVE_DECODE is unavailable in the isolated GSI v1 "
            "environment; use frozen expert residency"
        )


def kt_energy_on_decode_step() -> None:
    """Static-residency compatibility hook for the pinned model runner."""
    if os.environ.get("KT_ENERGY") == "1":
        raise RuntimeError(
            "KT_ENERGY is unavailable in the isolated GSI v1 environment; "
            "use frozen expert residency"
        )
